...
def combination(n, k, s=1, prev=[], first=True):
    if k == 0:
        return

    for i in range(s, n + 1 if not first else 2):
        new = prev + [i]
        comb.append(new)
        combination(n, k-1, i + 1, new, first=False)
...
for set in comb:
    ...
    for i in range(0, len(other_set)-1):
        for j in range(i+1, len(other_set)):
